...
// ConfigSnapshot returns a copy of the current config for read-only use.
func (m *Manager) ConfigSnapshot(ctx context.Context) (*config.Config, error) {
	if ctx != nil {
		if err := ctx.Err(); err != nil {
			return nil, err
		}
	...
	return m.copyConfigLocked(), nil
}

// UpdateSubscriptionConfig updates subscription URLs and refresh settings and persists them to disk.
func (m *Manager) UpdateSubscriptionConfig(ctx context.Context, subscriptions []string, refresh config.SubscriptionRefreshConfig) error {
	if ctx != nil {
		if err := ctx.Err(); err != nil {
			return err
		}
	...
	normalizedSubs := normalizeSubscriptionURLs(subscriptions)
	...
	prevSubs := cloneStrings(m.cfg.Subscriptions)
	prevRefresh := m.cfg.SubscriptionRefresh

	m.cfg.Subscriptions = normalizedSubs
	m.cfg.SubscriptionRefresh = refresh

	if err := m.cfg.SaveSubscriptionConfig(); err != nil {
		m.cfg.Subscriptions = prevSubs
		m.cfg.SubscriptionRefresh = prevRefresh
		return fmt.Errorf("save subscription config: %w", err)
	}
	return nil
}

// RefreshSubscriptions fetches nodes from configured subscriptions, writes them to nodes.txt,
// and reloads the running instance while preserving port assignments.
func (m *Manager) RefreshSubscriptions(ctx context.Context) (int, error) {
	if ctx != nil {
		if err := ctx.Err(); err != nil {
			return 0, err
		}
	...
	// Snapshot config for network fetch without holding the lock.
	m.mu.RLock()
	if m.cfg == nil {
		m.mu.RUnlock()
		return 0, errConfigUnavailable
	}
	subs := cloneStrings(m.cfg.Subscriptions)
	timeout := m.cfg.SubscriptionRefresh.Timeout
	configPath := m.cfg.FilePath()
	nodesFilePath := m.cfg.NodesFile
	portMap := m.cfg.BuildPortMap()
	m.mu.RUnlock()

	if len(subs) == 0 {
		return 0, errors.New("未配置订阅链接")
	}

	nodes, err := fetchAllSubscriptionNodes(subs, timeout)
	if err != nil {
		return 0, err
	}

	if nodesFilePath == "" {
		nodesFilePath = filepath.Join(filepath.Dir(configPath), "nodes.txt")
	}
	...
	backupNodes := cloneNodes(m.cfg.Nodes)
	backupNodesFile := m.cfg.NodesFile

	// Preserve inline nodes, replace everything else with fresh subscription nodes.
	...
	m.cfg.Nodes = append(cloneNodes(inlineNodes), nodes...)
	m.cfg.NodesFile = nodesFilePath

	if err := m.cfg.SaveNodes(); err != nil {
		m.cfg.Nodes = backupNodes
		m.cfg.NodesFile = backupNodesFile
		m.mu.Unlock()
		return 0, fmt.Errorf("save nodes: %w", err)
	}
...
func normalizeSubscriptionURLs(urls []string) []string {
...
func fetchAllSubscriptionNodes(subURLs []string, timeout time.Duration) ([]config.NodeConfig, error) {
	var all []config.NodeConfig
	var lastErr error
	for _, subURL := range subURLs {
		nodes, err := config.FetchNodesFromSubscription(subURL, timeout)
		if err != nil {
			lastErr = err
			continue
		}
	...
	if len(all) == 0 {
		...
		return nil, errors.New("订阅未返回任何节点")
	}
	for i := range all {
		all[i].Source = config.NodeSourceSubscription
	}
	return all, nil
}
...
func (m *Manager) copyConfigLocked() *config.Config {
	...
	cloned.Subscriptions = cloneStrings(m.cfg.Subscriptions)
